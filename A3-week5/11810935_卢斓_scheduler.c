...
void updateall()
{
    ...
    for (p = head; p != NULL; p = p->next) {
        p->job->wait_time += 1;
        p->job->curpri = p->job->defpri + p->job->wait_time;  // Priority add 1 after waiting for 100ms
	if (p->job->curpri > 3) {
	   p->job->curpri = 3;
	}
    }
...
struct waitqueue* jobselect()
{
    ...
    if (head) {
        ...
        select->next = NULL; // otherwise, reintroducing to the memory will cause a problem
    }
...
void jobswitch()
{
    ...
    if (next == NULL && current == NULL)          /* no job to run */  // current job done

        return;

    else if (next != NULL && current == NULL) {   /* start new job */  // current job done
    ...
    } else if (next != NULL && current != NULL) { /* do switch */  // current job not done

        kill(current->job->pid, SIGSTOP);
        current->job->curpri = current->job->defpri;  // Priority reset to initial priority after running.
    ...
    } else {    /* next == NULL && current != NULL, no switch */  // current job not done
...
int main()
{
    ...
    interval.tv_sec = 0;
    interval.tv_usec = 100000;  // 100000us = 100ms

    new.it_interval = interval;  /* Interval for periodic timer */
    new.it_value = interval;  /* Time waited until next expiration, only used for the first time*/
